...
public class GameCanvas extends Canvas implements Runnable {
	...
	private Image[] imgEnemy = new Image['¥'];
	...
	private Image[] imgAD = new Image[5];
	...
	private GameCanvas() {
		...
		try {
			this.imgTitle = Image.createImage("/res/title.GIF");
		...
		try {
			for (int i = 0; i < this.imgSeaBg.length; i++) {
				this.imgSeaBg[i] = Image.createImage("/res/seabg" + i + ".jpg");
			}

			for (int i = 0; i < this.imgBack.length; i++) {
				this.imgBack[i] = Image.createImage("/res/bg" + i + ".png");
			}

			this.imgPlayerSmall = Image.createImage("/res/ps.png");
			for (int i = 0; i < this.imgPS.length; i++) {
				this.imgPS[i] = Image.createImage(this.imgPlayerSmall, i * 15,
						0, 15, 12, 0);
				this.imgPlayer[i] = this.imgPS[i];
			}

			this.imgPlayerMiddle = Image.createImage("/res/pm.png");
			for (int i = 0; i < this.imgPM.length; i++) {
				this.imgPM[i] = Image.createImage(this.imgPlayerMiddle, i * 20,
						0, 20, 17, 0);
			...
			for (int j = 0; j < 11; j++) {
				this.imgEnemy0Small = Image.createImage("/res/e" + j + "s.png");
				for (int i = 0; i < 5; i++) {
					this.imgEnemy[(i + 15 * j)] = Image.createImage(
							this.imgEnemy0Small, i * 12, 0, 12, 10, 0);
				}

				this.imgEnemy0Middle = Image.createImage("/res/e" + j + "m.png");
				for (int i = 0; i < 5; i++) {
					this.imgEnemy[(5 + i + 15 * j)] = Image.createImage(
							this.imgEnemy0Middle, i * 17, 0, 17, 14, 0);
				}
			...
			for (int i = 0; i < this.imgBubble.length; i++) {
				this.imgBubble[i] = Image.createImage(imgBubbles, i * 16, 0,
						16, 16, 0);
			}
			...
			for (int i = 0; i < this.imgAD.length; i++) {
				this.imgAD[i] = Image.createImage("/res/ad" + i + ".jpg");
			}
	...
	private void drawMenu(Graphics g) {
		...
		if (menuIndex < 3){
			g.drawImage(this.imgMenu, this.screenWidth / 2, G.MAIN_MENU_TOP - menuIndex * 29, 17);
			
		}
		else {
			g.drawImage(this.imgMenu, this.screenWidth / 2, G.MAIN_MENU_TOP - (menuIndex + 2) * 29, 17);
		}
		

		g.setClip(0, 0, this.screenWidth, this.screenHeight);
		g.drawImage(this.imgArrow,
				G.computeX(118 + 2 * (this.counter % 8 / 4)), G.computeY(173),
				0);
		g.drawRegion(this.imgArrow, 0, 0, this.imgArrow.getWidth(),
				this.imgArrow.getHeight(), 2, G
						.computeX(50 - 2 * (this.counter % 8 / 4)), G
						.computeY(174), 0);
	}
	...
	private void drawHelp(Graphics g) {
		...
		g.drawString("2或上：向上移动。", helpX, helpY + 5 * this.lineSpace, 0);
		g.drawString("4或左：向左移动。", helpX, helpY + 6 * this.lineSpace, 0);
		g.drawString("6或右：向右移动。", helpX, helpY + 7 * this.lineSpace, 0);
		g.drawString("8或下：向下移动。", helpX, helpY + 8 * this.lineSpace, 0);
		g.drawString("数字键0：暂停游戏。", helpX, helpY + 9 * this.lineSpace, 0);
		...
		helpY -= 4;
		g.drawString("按确定键回到主菜单。", helpX, helpY + 16 * this.lineSpace, 0);
	}
	...
	private void drawIntro(Graphics g) {
		...
		g.setColor(16777215);
		g.drawString("第" + this.level + "关", this.screenWidth / 2, introY + 28,
				17);
		g.drawString("提示：", introX + 24, introY + 70, 0);
		if (this.level == 1)
			g.drawString("吃掉小鱼使自己成长。", introX + 24, introY + 92, 0);
		else if (this.level == 2)
			g.drawString("2x可在短时间内获得2倍积分。", introX + 24, introY + 92, 0);
		else if (this.level == 3)
			g.drawString("小心鲨鱼，不能碰到它们。", introX + 24, introY + 92, 0);
		else if (this.level == 4)
			g.drawString("进入新的海域。", introX + 24, introY + 92, 0);
		else if (this.level == 5)
			g.drawString("注意水母，碰到会被麻痹一段时间。", introX + 24, introY + 92, 0);
		else if (this.level == 6)
			g.drawString("雪花可以使所有敌人麻痹一段时间。", introX + 24, introY + 92, 0);
		else if (this.level == 7)
			g.drawString("进入新的海域，小心敌人。", introX + 24, introY + 92, 0);
		else if (this.level == 8)
			g.drawString("小心鲨鱼，已经接近胜利了。", introX + 24, introY + 92, 0);
		else if (this.level == 9) {
			g.drawString("注意大白鲨，完成整个游戏吧。", introX + 24, introY + 92, 0);
		}

		g.drawString("确定", introX + 24,
	...
	private void drawCongratulations(Graphics g) {
		...
		g.drawImage(this.imgIntro, introX, introY, 0);
		g.setColor(16777215);
		g.drawString("恭喜您完成游戏!", this.screenWidth / 2,
	...
	private void drawAnalys(Graphics g) {
		...
		g.fillRect(0, 0, this.screenWidth, this.screenHeight);
		int introX = this.screenWidth / 2 - this.imgIntro.getWidth() / 2;
		int introY = this.screenHeight / 2 - this.imgIntro.getHeight() / 2 - 16;
		g.drawImage(this.imgIntro, introX, introY + 16, 0);
		g.setColor(16777215);
		g.drawString("第" + this.level + "关统计", this.screenWidth / 2,
				introY + 48, 17);
		g.drawImage(this.imgEnemy[((this.small[(this.level - 1)] - 2) * 15)],
				introX + 24, introY + 92, 0);
		g.drawImage(this.imgEnemy[((this.middle[(this.level - 1)] - 2) * 15)],
				introX + 24, introY + 114, 0);
		g.drawImage(this.imgEnemy[((this.large[(this.level - 1)] - 2) * 15)],
				introX + 24, introY + 136, 0);
		g.drawImage(this.imgBy, introX + 40, introY + 92, 0);
		g.drawImage(this.imgBy, introX + 40, introY + 114, 0);
		g.drawImage(this.imgBy, introX + 40, introY + 136, 0);
		g.setClip(introX + 49, introY + 92, 6, 10);

		g.drawImage(this.imgNumber, introX + 49 - 6 * (this.smallEaten / 10),
				introY + 92, 0);
		g.setClip(introX + 55, introY + 92, 6, 10);
		g.drawImage(this.imgNumber, introX + 55 - 6 * (this.smallEaten % 10),
				introY + 92, 0);
		g.setClip(introX + 49, introY + 114, 6, 10);
		g.drawImage(this.imgNumber, introX + 49 - 6 * (this.middleEaten / 10),
				introY + 114, 0);
		g.setClip(introX + 55, introY + 114, 6, 10);
		g.drawImage(this.imgNumber, introX + 55 - 6 * (this.middleEaten % 10),
				introY + 114, 0);
		g.setClip(introX + 49, introY + 136, 6, 10);
		g.drawImage(this.imgNumber, introX + 49 - 6 * (this.largeEaten / 10),
				introY + 136, 0);
		g.setClip(introX + 55, introY + 136, 6, 10);
		g.drawImage(this.imgNumber, introX + 55 - 6 * (this.largeEaten % 10),
				introY + 136, 0);

		g.setClip(0, 0, this.screenWidth, this.screenHeight);
		g.drawString("确定", introX + 14,
				introY + this.imgIntro.getHeight() - 24, 0);
	}
	...
	private void drawBack(Graphics g) {
		...
		g.drawImage(this.imgSeaBg[((this.level - 1) / 3)], this.offsetX / 3
				...
				/ 2, 3);
		if (this.offsetX > 0) {
			g.drawImage(this.imgBack[((this.level - 1) / 3)], -464
					+ this.offsetX, this.screenHeight - 140 + this.offsetY, 0);
		}
		g.drawImage(this.imgBack[((this.level - 1) / 3)], this.offsetX,
				this.screenHeight - 140 + this.offsetY, 0);
		g.drawImage(this.imgBack[((this.level - 1) / 3)], 464 + this.offsetX,
				this.screenHeight - 140 + this.offsetY, 0);
	}
	...
	private void drawBoard(Graphics g) {
		...
		g.setColor(0);
		g.drawString("按0键返回", 478, 530 - this.imgAD[0].getHeight() - 24, 0);
		g.setColor(12756732);

		if ((G.AD_LEVEL >= 1) && (G.AD_LEVEL <= 5))
			g.drawImage(this.imgAD[(G.AD_LEVEL - 1)], 478,
					530 - this.imgAD[(G.AD_LEVEL - 1)].getHeight(), 0);
	}
	...
	private void drawLevelComplete(Graphics g) {
		g.setColor(16777215);
		g.drawString("第" + this.level + "关完成", this.screenWidth / 2,
				this.screenHeight / 2 - 8, 17);
	}
